import { ContainerBuilder, DiscordAPIError, MessageFlags, type Client } from "discord.js";
            ...
            user.accounts.map((account) =>
              accountQueue.add(async () => {
                ...
                if (config.env === "production" && user.allowData && result.code === 0) {
                  const rewards = result.data.awardIds.map(
                    (a) => result.data.resourceInfoMap[a.id]!,
                  );
            ...
            if (result.code !== 0) {
              ...
              if (extraRewards.length > 0) {
                container.addTextDisplayComponents(
                  ...
                  (t) =>
                    t.setContent(extraRewards.map((r) => `- ${r.name} x${r.count}`).join("\n")),
                );
              }
            }
          }
